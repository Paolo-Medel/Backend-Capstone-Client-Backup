...
export const Register = () => {
  ...
  const [is_business, setBusiness] = useState();
  const [cause_area] = useState([]);
    ...
    fetch(`http://localhost:8000/register`, {
      ...
      .then((authInfo) => {
        if (authInfo && authInfo.token) {
          localStorage.setItem("volunteer_token", JSON.stringify(authInfo));
          navigate("/");
        } else {
  ...
  return (
    <main className="container--login">
      <dialog className="dialog dialog--auth" ref={existDialog}>
        <div>Error: User Not Created</div>
        <button
      ...
      <section>
        <form className="form--login" onSubmit={handleRegister}>
          ...
          >
            <label htmlFor="is_business"> Are you a Business? </label>
            <div>
